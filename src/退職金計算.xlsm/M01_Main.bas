...
Attribute ƒ}ƒXƒ^[“Ç.VB_ProcData.VB_Invoke_Func = " \n14"
    ...
    strSTN = Sheets("‘ŞE‹àŒvZ").Range("AD5").Value  '–‹ÆŠ‹æ•ªiRH,RO,RT,TA,KAj
    strCD = Strings.Format(Sheets("‘ŞE‹àŒvZ").Range("G7").Value, "00000") 'Ğˆõº°ÄŞ
    strSQL = ""
    ...
    rsA.Open strSQL, cnA, adOpenStatic, adLockReadOnly
    If rsA.EOF Then
        Sheets("‘ŞE‹àŒvZ").Range("X15").Value = "–¢“o˜^‚Å‚·"  '–¼‘O
        Sheets("‘ŞE‹àŒvZ").Range("X16").Value = 0  '–{‹‹
        Sheets("‘ŞE‹àŒvZ").Range("X17").Value = 0  '‰Á‹‹
        Sheets("‘ŞE‹àŒvZ").Range("X18").Value = "" '¶”NŒ“ú
        Sheets("‘ŞE‹àŒvZ").Range("X19").Value = "" '“üĞ“ú
    Else
